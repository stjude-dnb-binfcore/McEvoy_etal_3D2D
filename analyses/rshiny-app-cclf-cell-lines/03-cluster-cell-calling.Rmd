...
#analysis_dir <- file.path(root_dir, "analyses", "rshiny-app-cclf-cell-lines") 
#data_dir <- file.path(root_dir, "analyses", "rshiny-app-cclf-cell-lines", "results")
...
source(paste0(module_dir, "/util/function-cluster-cell-calling.R"))
```
...
# Read seurat object
First, we will use the `seurat_obj_integrated_{integration_method}.rds` object as generated from the pipeline in the `integrative-analysis` module. This object contains all samples of the project integrated by the integration method of choice as defined in the `params`. There we have already applied the `FindClusters` function but here we will explore and expand the cell clustering further.
...
if (length(resolution_list) > 1 | is.null(resolution_list)) {
  name <- paste0(plots_dir, "/", glue::glue("plot_clustree_overlay_{integration_method}.png"))
